...
const TermsPageClient = ({ lang }: { lang: LangCode }) => {
  const t = useTranslations();

  const sections = t.raw('legal.terms.sections') as Array<{ title: string; content: string }>;
  const title = t('legal.terms.title');
  const lastUpdated = t('legal.terms.lastUpdated');
  const version = t('legal.terms.version');
