...
const PlayerCard = ({ player, lang }: PlayerCardProps) => {
  ...
  const playerDetailPath = `/${lang}/${localizedRouteNames.players[lang]}/${player.id}`;
  ...
  const handleToggleFavorite = async (e: React.MouseEvent) => {
    ...
    try {
      const newFavorites = isFavorited
        ? favoritePlayers.filter((id: string) => id !== player.id)
  ...
  return (
    <Link href={playerDetailPath} className="block h-full">
