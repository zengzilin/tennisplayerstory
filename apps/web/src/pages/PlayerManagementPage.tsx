...
import PlayerTableRow from '@/components/PlayerTableRow.tsx';
...
const PlayerManagementPage = () => {
  ...
  const handleImportConfirm = async (playersToImport, isTest = false) => {
    ...
    for (let i = 0; i < runList.length; i++) {
      ...
      console.log(`[IMPORT] Row index ${i} raw Excel data:`, p);
      console.log(`[IMPORT] Extracted source value before saving:`, src);
      ...
      try {
        ...
        if (existing.items.length > 0) {
          ...
          console.log('[IMPORT] Source value being sent to pb.collection("players").create():', payload.source);
