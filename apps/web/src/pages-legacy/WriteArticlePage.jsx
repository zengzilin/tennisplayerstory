...
const WriteArticlePage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // 3. Check that author is correctly set to currentUser.id
    if (!currentUser?.id) {
    ...
    try {
      // 2, 4, 5, 6. Match exact schema, capture featured player, set pending, proper types
      const articleData = {
        title: formData.title,
        player_name: formData.player_name,
        content: formData.content,
        author: currentUser.id, // String ID for relation
        status: 'pending'
      };
      ...
      // 9. Ensure success toast appears and form resets
      toast.success(t('articles.submitSuccess'));
      setFormData({ title: '', player_name: '', content: '' });
      navigate('../my-articles');
      
    } catch (err) {
      // 7. Add detailed error logging to catch block
      ...
      // Extract specific error message if provided by PocketBase
  ...
  return (
    <>
      <Helmet>
        <title>{t('articles.writeTitle')} - TennisHub</title>
      </Helmet>
