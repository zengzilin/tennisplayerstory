  ...
  return (
    <>
      <Helmet>
        ...
        <meta name="description" content={t('profile.editDesc', 'Manage your profile')} />
      </Helmet>
                        ...
                        <DialogContent className="sm:max-w-[425px]">
                          <form onSubmit={handleSaveProfile}>
                            <DialogHeader>
                              ...
                              <DialogDescription>
                                {t('profile.editDesc', 'Update your profile details')}
                              </DialogDescription>
                            </DialogHeader>
            ...
            >
              <Tabs defaultValue="articles" className="w-full">
                <TabsList className="grid w-full max-w-md grid-cols-2 mb-8">
                  <TabsTrigger value="articles" className="text-base">{t('profile.myArticles', 'My Articles')}</TabsTrigger>
                  <TabsTrigger value="favorites" className="text-base">{t('profile.favPlayers', 'Favorite Players')}</TabsTrigger>
                </TabsList>

                <TabsContent value="articles" className="space-y-6 focus-visible:outline-none focus-visible:ring-0">
                  <div className="flex items-center justify-between">
                    <h2 className="text-2xl font-bold">{t('profile.myArticles', 'My Articles')}</h2>
                    <Button onClick={() => navigate('/write-article')} size="sm">
                  ...
                  {articlesLoading ? (
                    ...
                    <Card className="border-dashed border-2 bg-transparent">
                      <CardContent className="flex flex-col items-center justify-center py-16 text-center">
                        ...
                        <h3 className="text-xl font-semibold mb-2">{t('profile.noArticlesTitle', 'No Articles')}</h3>
                        <p className="text-muted-foreground max-w-md mb-6">
                          {t('profile.noArticlesDesc', 'You have not written any articles yet.')}
                        </p>
                ...
                <TabsContent value="favorites" className="space-y-6 focus-visible:outline-none focus-visible:ring-0">
                  <div className="flex items-center justify-between">
                    <h2 className="text-2xl font-bold">{t('profile.favPlayers', 'Favorite Players')}</h2>
                    <span className="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-semibold bg-secondary text-secondary-foreground">
                      {favoritePlayers.length} {t('profile.saved', 'Saved')}
                    </span>
                  </div>
                  ...
                  {playersLoading ? (
                    ...
                    <Card className="border-dashed border-2 bg-transparent">
                      <CardContent className="flex flex-col items-center justify-center py-16 text-center">
                        ...
                        <h3 className="text-xl font-semibold mb-2">{t('profile.noFavsTitle', 'No Favorites')}</h3>
                        <p className="text-muted-foreground max-w-md mb-6">
                          {t('profile.noFavsDesc', 'You have not saved any favorite players yet.')}
                        </p>
                        <Button onClick={() => navigate('/players')}>
                          {t('profile.browsePlayers', 'Browse Players')}
                        </Button>
                      </CardContent>
                    </Card>
                  )}
      ...
      <Dialog open={!!editingArticle} onOpenChange={(open) => !open && setEditingArticle(null)}>
        <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
          <form onSubmit={handleEditArticleSubmit}>
            <DialogHeader>
              <DialogTitle>{t('profile.editArticle', 'Edit Article')}</DialogTitle>
            </DialogHeader>
            <div className="grid gap-4 py-4">
              <div className="space-y-2">
                <Label htmlFor="article-title">{t('profile.articleTitle', 'Title')}</Label>
                <Input
                  id="article-title"
              ...
              <div className="space-y-2">
                <Label htmlFor="article-tags">{t('profile.articleTags', 'Tags')}</Label>
                <Input
                  id="article-tags"
              ...
              <div className="space-y-2">
                <Label htmlFor="article-content">{t('profile.articleContent', 'Content')}</Label>
                <Textarea
                  id="article-content"
      ...
      <AlertDialog open={!!deletingArticle} onOpenChange={(open) => !open && setDeletingArticle(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t('profile.confirmDeleteTitle', 'Confirm Delete')}</AlertDialogTitle>
            <AlertDialogDescription>
              {t('profile.confirmDeleteDesc', 'Are you sure you want to delete this article?')}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
