...
const SCRAPE_LOG_SOURCES = {
  atp: 'ATP',
  wta: 'WTA',
  itf: 'ITF',
};

const getLogSource = (source) => SCRAPE_LOG_SOURCES[source] || String(source || '').toUpperCase();
...
function parseEspnRankings($, source) {
  const normalizedSource = PLAYER_SOURCES[source] || source;
  const players = [];
...
router.post('/atp', async (req, res) => {
  const source = PLAYER_SOURCES.atp;
  ...
  try {
    ...
    const players = parseEspnRankings($, source);
    ...
    // Sync to database
    const syncResult = await syncPlayerData(players, source);
...
router.post('/wta', async (req, res) => {
  const source = PLAYER_SOURCES.wta;
  const logSource = getLogSource(source);
  const timestamp = new Date().toISOString();
  let htmlSnippet = '';
...
router.post('/itf', async (req, res) => {
  const source = PLAYER_SOURCES.itf;
  const logSource = getLogSource(source);
  const timestamp = new Date().toISOString();
  let htmlSnippet = '';
...
router.post('/all', async (req, res) => {
  ...
  try {
    ...
    const atpPlayers = parseEspnRankings(cheerio.load(atpHtml), PLAYER_SOURCES.atp);

    if (atpPlayers.length > 0) {
      const syncResult = await syncPlayerData(atpPlayers, PLAYER_SOURCES.atp);
      results.data.atp = { count: atpPlayers.length, created: syncResult.created, updated: syncResult.updated };
      await pb.collection('scrape_logs').create({
        source: getLogSource(PLAYER_SOURCES.atp),
        status: 'success',
  ...
  try {
    ...
    wtaRows.each((index, element) => {
      try {
        ...
        const ageText = $row.find('td.player-row__cell--age').text().trim();
        const player = { name, ranking: parseInt(ranking) || 0, country, points: parseInt(points) || 0, age: ageText ? parseInt(ageText) : null, profileUrl: '', source: PLAYER_SOURCES.wta };
    ...
    if (wtaPlayers.length > 0) {
      const syncResult = await syncPlayerData(wtaPlayers, PLAYER_SOURCES.wta);
      results.data.wta = { count: wtaPlayers.length, created: syncResult.created, updated: syncResult.updated };
      await pb.collection('scrape_logs').create({
        source: getLogSource(PLAYER_SOURCES.wta),
        status: 'success',
  ...
  try {
    ...
    const itfPlayers = parseEspnRankings(cheerio.load(itfHtml), PLAYER_SOURCES.itf);

    if (itfPlayers.length > 0) {
      const syncResult = await syncPlayerData(itfPlayers, PLAYER_SOURCES.itf);
      results.data.itf = { count: itfPlayers.length, created: syncResult.created, updated: syncResult.updated };
      await pb.collection('scrape_logs').create({
        source: getLogSource(PLAYER_SOURCES.itf),
        status: 'success',
...
router.get('/status', async (req, res) => {
  ...
  try {
    ...
    logs.items.forEach(log => {
      const normalizedSource = String(log.source || '').trim().toLowerCase();
      if (normalizedSource && !sourceMap[normalizedSource]) {
        sourceMap[normalizedSource] = log.timestamp;
      }
    });

    status.atp = sourceMap[PLAYER_SOURCES.atp] || null;
    status.wta = sourceMap[PLAYER_SOURCES.wta] || null;
    status.itf = sourceMap[PLAYER_SOURCES.itf] || null;
  } catch (error) {
